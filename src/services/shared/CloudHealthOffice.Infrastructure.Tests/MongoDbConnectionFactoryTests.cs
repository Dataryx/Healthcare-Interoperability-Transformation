...
public class MongoDbConnectionFactoryTests
{
    ...
    [Fact]
    public void AddChoInfrastructure_RegistersMongoFactoryAndDatabaseAsScoped()
    {
        ...
        services.AddChoInfrastructure(configuration, options => options.ServiceName = "Test Service");

        var factoryDescriptor = services.Single(d => d.ServiceType == typeof(MongoDbConnectionFactory));
        var databaseDescriptor = services.Single(d => d.ServiceType == typeof(IMongoDatabase));
