...
/// <summary>
/// Shared setup for the Cloud Health Office platform. This keeps the common
/// infrastructure wiring consistent across the services instead of repeating the
/// same setup in each app.
/// </summary>
public static class ServiceCollectionExtensions
{
    /// <summary>
    /// Registers the shared infrastructure for a service, including health checks,
    /// CORS, Swagger, and the database connection used by that app.
    /// </summary>
    public static IServiceCollection AddChoInfrastructure(this IServiceCollection services, IConfiguration configuration, Action<ChoInfrastructureOptions>? configure = null)
    {
        ...
        // Health checks are auto-detected from configuration so the service can
        // verify its local dependencies without custom wiring in each app.
        ...
        // Use a custom CORS policy when one is supplied; otherwise, allow requests
        // from anywhere for local and shared service setups.
        ...
        // Swagger is enabled for services that have a UI name and description.
        ...
        // Register the database connection based on the app's configuration.
        RegisterDatabase(services, configuration);

        // Keep the tenant middleware settings available when the app starts.
        services.AddSingleton(options.TenantOptions);

        // Keep the CORS policy name for the middleware pipeline.
    ...
    public static IApplicationBuilder UseChoInfrastructure(this IApplicationBuilder app, IConfiguration configuration)
    {
        ...
        // Catch exceptions as early as possible so the app responds consistently.
        ...
        // Resolve tenant settings before handling each request.
        ...
        // Expose the platform health endpoints for liveness and readiness checks.
