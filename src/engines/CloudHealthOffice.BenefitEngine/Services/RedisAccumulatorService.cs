...
/// <summary>
/// Accumulator balances live in Redis hashes, rebuilt from finalized claim
/// lines when the key is missing. We do not store a mutable "current balance"
/// document — two claims for the same member will race that every time.
///
/// Key: accum:{tenantId}:{memberId}:{planId}:{planYear}
/// Field: {AccumulatorType}:{NetworkTier} → amount
/// Family rows use subscriberId (FAM:{id}) instead of memberId.
///
/// HINCRBYFLOAT is the write path so concurrent adjudication does not need
/// a read-modify-write lock. Voids send negative increments. If Redis dropped
/// the key, the next read sums claim history (which already excludes the void).
///
/// This does not go through ICacheProvider. That abstraction is string get/set;
/// we need hash atomics. See docs/architecture/shared-cache.md.
/// </summary>
